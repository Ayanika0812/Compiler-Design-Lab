...
void parse() {
    ...
    while(1) {
        ...
        if(action > 0) { 
            ...
            switch(prod) {
                case 0: printf("E → E + T"); break;
                case 1: printf("E → T"); break;
                case 2: printf("T → T * F"); break;
                case 3: printf("T → F"); break;
                case 4: printf("F → ( E )"); break;
                case 5: printf("F → id"); break;
            }
            printf(")\n");
            
            int rhs_len = productions[prod][1];
            for(int i=0; i<rhs_len; i++) pop();
            
            int lhs = productions[prod][0];
            int new_state = goto_table[stack[top]][lhs - E];
            push(new_state);
        }
        else if(action == 0) {
...
int main() {
    int sample_input[] = {ID, PLUS, ID, STAR, STAR, DOLLAR};
    
    printf("Input string: id+id**$\n");
    for(int i = 0; i < 6; i++) {
        input[i] = sample_input[i];
    }
    
    printf("Parsing steps for 'id+id**$':\n");
    parse();
    return 0;
}
